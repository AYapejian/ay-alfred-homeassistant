...
| 2026-09-24 | **Server profiles: hybrid config + Keychain** | The default server stays in Alfred's workflow configuration (`HA_URL`/`HA_TOKEN`, zero-action upgrade); added servers live in `<data>/profiles.json` with no secrets, and their tokens in the login Keychain, written through `security -i` on stdin so a token never appears in argv. Alfred has no list-valued config field, and numbered slots would keep every token in a synced plaintext file. Trade-off: added servers are per-Mac. The active server resolves only in `Config.from_env()` and fails closed; every item carries its server id so an action never lands on another house. Spec: `specs/spec-001-server-profiles.md`. |
